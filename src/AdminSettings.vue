...
<template>
    <div id="registration_settings_form">
        <NcSettingsSection :title="t('registration', 'Registration settings')">
            <NcCheckboxRadioSwitch :checked.sync="adminApproval" type="switch" :disabled="loading"
                @update:checked="saveData">
                {{ t('registration', 'Require administrator approval') }}
            </NcCheckboxRadioSwitch>

            <p><em>{{ t('registration', 'Enabling "administrator approval" will prevent registrations from mobile
            and desktop clients to complete since the credentials cannot be verified by the client until the user was
            enabled.') }}</em></p>
            ...
            <div>
                ...
                <NcMultiselect id="registered_user_group" v-model="registeredUserGroup"
                    :placeholder="t('registration', 'Select group')" :options="groups" :disabled="loading"
                    :searchable="true" :tag-width="60" :loading="loadingGroups" :allow-empty="true"
                    :close-on-select="false" track-by="id" label="displayname" @search-change="searchGroup"
                    @change="saveData" />
            </div>
        </NcSettingsSection>

        <NcSettingsSection :title="t('registration', 'Email settings')">
            <NcCheckboxRadioSwitch :checked.sync="emailIsOptional" type="switch" :disabled="loading"
                @update:checked="saveData">
                {{ t('registration', 'Email is optional') }}
            </NcCheckboxRadioSwitch>

            <NcTextField :label="domainListLabel" :label-visible="true" :value.sync="allowedDomains" :disabled="loading"
                placeholder="nextcloud.com;*.example.com" @input="debounceSavingSlow" />

            <NcCheckboxRadioSwitch :checked.sync="domainsIsBlocklist" type="switch" :disabled="loading"
                @update:checked="saveData">
                {{ t('registration', 'Block listed email domains instead of allowing them') }}
            </NcCheckboxRadioSwitch>

            <NcCheckboxRadioSwitch :checked.sync="showDomains" type="switch" :disabled="loading"
                @update:checked="saveData">
                {{ showDomainListLabel }}
            </NcCheckboxRadioSwitch>

            <NcCheckboxRadioSwitch v-if="!emailIsOptional" :checked.sync="disableEmailVerification" type="switch"
                :disabled="loading" @update:checked="saveData">
                {{ t('registration', 'Disable email verification') }}
            </NcCheckboxRadioSwitch>
        </NcSettingsSection>

        <NcSettingsSection :title="t('registration', 'User settings')">
            <NcCheckboxRadioSwitch v-if="!emailIsOptional" :checked.sync="emailIsLogin" type="switch"
                :disabled="loading" @update:checked="saveData">
            ...
            <template v-if="!emailIsLogin">
                <p>
                    ...
                    <input id="username_policy_regex" v-model="usernamePolicyRegex" type="text"
                        name="username_policy_regex" :disabled="loading" placeholder="E.g.: /^[a-z-]+\.[a-z-]+$/"
                        :aria-label="t('registration', 'Regular expression to validate login names')"
                        @input="debounceSavingSlow">
                </p>
                <em>{{ t('registration', 'If configured, login names will be validated through the regular expression.
                If the validation fails the user is prompted with a generic error.Make sure your regex is working
                correctly.') }}</em>
            </template>

            <NcCheckboxRadioSwitch :checked.sync="showFullname" :disabled="loading" type="switch"
                @update:checked="saveData">
                {{ t('registration', 'Show full name field') }}
            </NcCheckboxRadioSwitch>

            <NcCheckboxRadioSwitch v-if="showFullname" class="indent" :checked.sync="enforceFullname" type="switch"
                :disabled="loading" @update:checked="saveData">
                {{ t('registration', 'Enforce full name field') }}
            </NcCheckboxRadioSwitch>

            <NcCheckboxRadioSwitch v-if="canShowPhone" :checked.sync="showPhone" type="switch" :disabled="loading"
                @update:checked="saveData">
                {{ t('registration', 'Show phone field') }}
            </NcCheckboxRadioSwitch>

            <NcCheckboxRadioSwitch v-if="canShowPhone && showPhone" class="indent" :checked.sync="enforcePhone"
                type="switch" :disabled="loading" @update:checked="saveData">
        ...
        <NcSettingsSection :title="t('registration', 'User instructions')"
            ...
            <input v-model="additionalHint" type="text" name="additional_hint" :disabled="loading"
                ...
                @input="debounceSavingSlow">
            <p><em>{{ t('registration', 'Add additional user instructions (e.g. for choosing their login name). If
            configured the text is displayed in the account creation step of the registration process.') }}</em>
            </p>

            <h3>{{ t('registration', 'Verification email instructions') }}</h3>
            <input v-model="emailVerificationHint" type="text" name="email_verification_hint" :disabled="loading"
                placeholder="Please create your username following the scheme 'firstname.lastname'."
                :aria-label="t('registration', 'A short message that is shown to the user in the verification email.')"
                @input="debounceSavingSlow">
            <p><em>{{ t('registration', 'Add additional user instructions (e.g. for choosing their login name). If
            configured the text is embedded in the verification - email.') }}</em></p>
        </NcSettingsSection>
    </div>
</template>
