    ...
    <div class="guest-box">
        <form action="" method="post" @submit="onSubmit">
            ...
            <fieldset>
                ...
                <NcTextField v-if="!emailIsOptional || email.length > 0" :value.sync="email" type="email"
                    :label="t('registration', 'Email')" :label-visible="true" name="email" disabled>
                    <Email :size="20" class="input__icon" />
                </NcTextField>

                <NcTextField v-if="!emailIsLogin" :value.sync="loginname" type="text" name="loginname"
                    :label="t('registration', 'Login name')" :label-visible="true" required>
                    <Key :size="20" class="input__icon" />
                </NcTextField>
                <input v-else type="hidden" name="loginname" :value="email">

                <NcTextField v-if="showFullname" :value.sync="fullname" type="text" name="fullname"
                    :label="t('registration', 'Full name')" :label-visible="true" :required="enforceFullname">
                    <Account :size="20" class="input__icon" />
                </NcTextField>
                <input v-else type="hidden" name="fullname" value="">

                <NcTextField v-if="showPhone" :value.sync="phone" type="text" name="phone"
                    :label="t('registration', 'Phone number')" :label-visible="true" :required="enforcePhone">
                    <Phone :size="20" class="input__icon" />
                </NcTextField>
                <input v-else type="hidden" name="phone" value="">

                <NcPasswordField :value.sync="password" :label="t('registration', 'Password')" :label-visible="true"
